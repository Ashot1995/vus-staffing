<?php

return [
    // Seo
    'seo.home.title' => 'VUS Bemanning – Staffing and Recruitment in Luleå & Norrbotten',
    'seo.home.description' => 'VUS Bemanning is your partner for staffing and recruitment in Luleå and Norrbotten. Professional service, broad competence, and fast matching. Contact us today.',
    'seo.jobs.description' => 'Search available jobs at VUS Bemanning. Staffing and recruitment in Luleå and Norrbotten. Find your next job.',
    'seo.contact.description' => 'Contact VUS Bemanning. We help with staffing and recruitment in Luleå and Norrbotten.',
    'seo.about.description' => 'Learn more about VUS Bemanning. Your partner for staffing and recruitment in Norrbotten.',
    'seo.employers.description' => 'Staffing and recruitment for companies. Contact VUS Bemanning for professional staff in Luleå.',
    'seo.blog.description' => 'Latest news and insights from VUS Bemanning. Staffing and recruitment in Norrbotten.',
    'seo.company_values.description' => 'VUS Bemanning – our core values. Professional staffing and recruitment.',
    'seo.candidate_info.description' => 'Information for job seekers at VUS Bemanning. Staffing and recruitment in Luleå.',
    'seo.spontaneous.description' => 'Submit a spontaneous application to VUS Bemanning. We will contact you when a suitable position becomes available.',

    // About
    'about.cta.button' => 'Contact us',
    'about.cta.title' => 'Want to know more?',
    'about.ect.description' => 'We take responsibility all the way – from the first dialogue to the completed assignment. At V U S you get a partner who is close, solution-oriented and clear in communication. We are meticulous in matching, quick when needed and present throughout the assignment. For us, quality is not a one-off effort, but a way of working where we support, follow up and act immediately if something needs to be adjusted. The goal is always the same: a delivery that feels safe, professional and well-executed for both the customer and the employee.',
    'about.ect.title' => 'Our mission',
    'about.gol.description' => 'Our goal is to make staffing and recruitment simple, safe and accurate through a working method that is based on responsibility, clarity and follow-up. We start from the customer\'s real needs and work environment and match based on competence, experience and attitude.
Every assignment should start with clear prerequisites, the right introduction and ongoing follow-up so that everything works well in practice. At the same time, we create safe conditions for our employees, which allows them to perform sustainably, develop and feel pride in their assignments.',
    'about.gol.title' => 'Gol',
    'about.our_version.description' => 'Our vision is to offer staffing and recruitment services that are as professional as they are human. We want to be the obvious partner for companies that need the right skills and for people who are looking for a secure employer and meaningful assignments. For us, success is not just about filling a position quickly, but about creating long-term solutions where both customers and employees feel confident. When we match the right person to the right place, quality, job satisfaction and results that last over time are created.',
    'about.our_version.title' => 'Our vision',
    'about.subtitle' => 'Your partner for staffing and recruitment',
    'about.team.member1.name' => 'Abdulrazek Mahmoud',
    'about.team.member1.role' => 'Founder & Chairman of the board',
    'about.team.member1.title' => 'Consulting Manager
',
    'about.team.member2.name' => 'Hrayr Hovhannisyan',
    'about.team.member2.role' => 'Founder & Board member',
    'about.team.member2.title' => 'Project Manager',
    'about.team.member3.name' => 'Abdulhamid Madi',
    'about.team.member3.role' => 'Founder & Board Member',
    'about.team.member3.title' => 'Site manager',
    'about.team.subtitle' => 'Here you can read more about us.',
    'about.team.title' => 'Who are we?',
    'about.title' => 'About V U S',
    'about.values.professionalism' => 'Professionalism in all contacts',
    'about.values.quality' => 'Quality over quantity',
    'about.values.relationships' => 'Long-term relationships',
    'about.values.service' => 'Personal service',
    'about.values.title' => 'Our core values',
    'about.values.transparency' => 'Transparency and honesty',
    'about.vision.description' => 'To be the obvious choice for companies and individuals seeking qualified personnel or new career opportunities.',
    'about.vision.title' => 'Our vision',
    'about.welcome.description' => 'V U S stands for value, excellent and safe work.',
    'about.welcome.description2' => 'V U S was founded with high ambitions to be both a professional and humane employer. To achieve this goal, people need to feel good – both at work and outside of work. In our assignments, our employees should develop, take responsibility and strengthen their positive qualities. We know that people are the most important resource in every organization, and therefore our focus is always the same: the right person with the right skills and the right attitude in the right place.',
    'about.welcome.description3' => 'We offer tailored services in recruitment and staffing for both long-term and seasonal assignments.',
    'about.welcome.title' => 'Welcome to V U S',

    // Admin
    'admin.application.address' => 'Address',
    'admin.application.applicant' => 'Applicant',
    'admin.application.applied_date' => 'Application Date',
    'admin.application.availability' => 'Availability',
    'admin.application.b_license' => 'B-License',
    'admin.application.consent' => 'Consent',
    'admin.application.consent.full' => 'Full consent - Can be used for matching with other jobs',
    'admin.application.consent.full_short' => 'Full',
    'admin.application.consent.limited' => 'Limited consent - Only for this application',
    'admin.application.consent.limited_short' => 'Limited',
    'admin.application.consent_type' => 'Consent Type',
    'admin.application.current_cv' => 'Current CV',
    'admin.application.current_image' => 'Current image',
    'admin.application.date_of_birth' => 'Date of Birth',
    'admin.application.download_cv' => 'Download CV',
    'admin.application.driving_license_b' => 'B Driving License',
    'admin.application.driving_license_category_1' => 'C-körkort',
    'admin.application.driving_license_category_2' => 'CE-körkort',
    'admin.application.driving_license_category_3' => 'Giltig ADR',
    'admin.application.driving_license_category_4' => 'Giltig YKB',
    'admin.application.driving_license_category_5' => 'Truck',
    'admin.application.driving_license_category_6' => 'Kran',
    'admin.application.email' => 'Email',
    'admin.application.expected_start' => 'Expected Start',
    'admin.application.first_name' => 'First Name',
    'admin.application.job' => 'Job',
    'admin.application.job_position' => 'Job Position',
    'admin.application.last_updated' => 'Last updated',
    'admin.application.no_cv' => 'No CV uploaded',
    'admin.application.other' => 'Other information',
    'admin.application.own_car' => 'Own car',
    'admin.application.phone' => 'Phone',
    'admin.application.spontaneous' => 'Spontaneous Application',
    'admin.application.start_date' => 'Expected Start Date',
    'admin.application.surname' => 'Surname',
    'admin.application.type' => 'Type',
    'admin.application.upload_cv' => 'Upload/Update CV',
    'admin.application.upload_image' => 'Upload/Update Personal image',
    'admin.application.view_cv' => 'View CV',
    'admin.contact.address_en' => 'Address',
    'admin.contact.address_sv' => 'Address',
    'admin.contact.created_from' => 'Created from',
    'admin.contact.created_until' => 'Created until',
    'admin.contact.email' => 'Email',
    'admin.contact.hours_weekdays_en' => 'Weekdays hours (English)',
    'admin.contact.hours_weekdays_sv' => 'Weekdays hours (Swedish)',
    'admin.contact.hours_weekend_en' => 'Weekend Hours (English)',
    'admin.contact.hours_weekend_sv' => 'Weekend hours (Swedish)',
    'admin.contact.message' => 'Message',
    'admin.contact.name' => 'Name',
    'admin.contact.phone' => 'Phone',
    'admin.contact.subject' => 'Subject',
    'admin.contact.submitted_at' => 'Sent',
    'admin.page_images.alt_text' => 'Alt Text',
    'admin.page_images.upload' => 'Upload image',
    'admin.partner.active' => 'Active',
    'admin.partner.active_only' => 'Active only',
    'admin.partner.created' => 'Created',
    'admin.partner.description' => 'Description',
    'admin.partner.inactive_only' => 'Inactive only',
    'admin.partner.logo' => 'Partner Logo',
    'admin.partner.name' => 'Partner Name',
    'admin.partner.order' => 'Order',
    'admin.partner.sort_order' => 'Sort Order',
    'admin.partner.website' => 'Website URL',
    'admin.status.accepted' => 'Accepted',
    'admin.status.pending' => 'Pending',
    'admin.status.rejected' => 'Rejected',
    'admin.status.reviewed' => 'Reviewed',
    'admin.status.shortlisted' => 'Shortlisted',

    // Apply
    'apply.additional_information' => 'Additional information (Optional)',
    'apply.additional_information_placeholder' => 'Specify selected license (e.g. C2, A2, ADR, truck-mounted or radio-controlled crane) and any additional information you wish to share.',
    'apply.address' => 'Full address',
    'apply.apply_to' => 'Apply to',
    'apply.availability' => 'Availability',
    'apply.cancel' => 'Cancel',
    'apply.consent.full' => 'I hereby consent that the information I provide in this application may be used to match me with this and other jobs.',
    'apply.consent.limited' => 'I provide limited consent, which means that the information I provide will only be used in this application and not to match me with other jobs.',
    'apply.consent.required' => 'Please select a consent option',
    'apply.cover_letter' => 'Cover letter',
    'apply.cover_letter_placeholder' => 'Tell us briefly why you are the right person for this job...',
    'apply.cv_invalid_format' => 'CV must be a PDF, DOC, or DOCX file',
    'apply.cv_required' => 'CV file is required',
    'apply.cv_too_large' => 'CV file size must be less than 3 MB',
    'apply.cv_upload_failed' => 'The CV failed to upload',
    'apply.date_of_birth' => 'Date of birth',
    'apply.day' => 'Day',
    'apply.documents_help' => 'You can upload up to 3 files, maximum 3 MB each. For multiple files: upload them at the same time.',
    'apply.driving_license_b' => 'B driving license',
    'apply.driving_license_category_1' => 'C driving license',
    'apply.driving_license_category_2' => 'CE driving license',
    'apply.driving_license_category_3' => 'Valid ADR ',
    'apply.driving_license_category_4' => 'Valid YKB ',
    'apply.driving_license_category_5' => 'Forklifts icense',
    'apply.driving_license_category_6' => 'Crane license',
    'apply.driving_license_own_car' => 'Own car',
    'apply.driving_license_privileges' => 'Licence categories',
    'apply.email' => 'Email',
    'apply.file_size_notice' => 'The maximum file size is 3 MB for each file, such as a CV and other certificates.',
    'apply.first_name' => 'First name',
    'apply.gdpr_consent' => 'I consent to my personal data being processed in accordance with GDPR.',
    'apply.is_18_or_older' => 'Are you 18 years old or will you turn 18 at the latest at the time of possible employment?',
    'apply.is_18_or_older.no' => 'No',
    'apply.is_18_or_older.yes' => 'Yes',
    'apply.month' => 'Month',
    'apply.optional' => 'Optional',
    'apply.other' => 'Other',
    'apply.other_placeholder' => 'Any additional information...',
    'apply.personal_image_notice' => 'Upload your personal photo (optional)',
    'apply.personal_info' => 'Personal Information',
    'apply.phone' => 'Phone',
    'apply.phone_help' => 'Select your country and enter your phone number',
    'apply.phone_placeholder' => 'Enter phone number',
    'apply.required' => 'Required',
    'apply.start_date' => 'When can you start work?',
    'apply.start_date_custom' => 'Select custom date',
    'apply.start_date_option.immediately' => 'Immediately',
    'apply.start_date_option.one_month' => 'Within a month',
    'apply.start_date_option.two_three_months' => 'Within two to three months',
    'apply.start_date_placeholder' => 'Select availability',
    'apply.submit' => 'Send',
    'apply.surname' => 'Surname',
    'apply.title' => 'Apply',
    'apply.upload_cv' => 'Upload CV (PDF, DOC, DOCX)',
    'apply.upload_documents' => 'Upload documents (PDF, DOC, DOCX, Images)',
    'apply.upload_personal_image' => 'Upload Personal image',
    'apply.uploads' => 'Uploads',
    'apply.year' => 'Year',

    // Auth
    'auth.admin_panel' => 'Admin Panel',
    'auth.already_registered' => 'Already registered?',
    'auth.email' => 'Email',
    'auth.forgot_password' => 'Forgot your password?',
    'auth.login' => 'Log in',
    'auth.name' => 'Name',
    'auth.password' => 'Password',
    'auth.password_confirmation' => 'Confirm password',
    'auth.register' => 'Register',
    'auth.remember_me' => 'Remember me',

    // Common
    'common.contact_us' => 'Contact us',
    'common.country.sweden' => 'Sweden',
    'common.learn_more' => 'Learn more',
    'common.read_more' => 'Read more',
    'common.video_not_supported' => 'Your browser does not support the video tag.',
    'common.view_jobs' => 'View jobs',

    // Company values
    'company_values.no_values.description' => 'Company values will be displayed here once they are added.',
    'company_values.no_values.title' => 'No Company Values Available',
    'company_values.subtitle' => 'Our core values that guide everything we do',
    'company_values.title' => 'Company Values',

    // Candidate info
    'candidate_info.no_items.description' => 'Candidate information will be displayed here once it is added.',
    'candidate_info.no_items.title' => 'No Information Available',
    'candidate_info.subtitle' => 'Important information for job seekers',
    'candidate_info.title' => 'Candidate Information',

    // Contact
    'contact.address' => 'Address',
    'contact.contact_info' => 'Contact Information',
    'contact.email' => 'Email',
    'contact.form.message' => 'Message',
    'contact.form.name' => 'Name',
    'contact.form.phone' => 'Phone',
    'contact.form.privacy_consent_text' => 'I agree to the',
    'contact.form.response_time' => 'We\'ll get back to you within 24 hours.',
    'contact.form.subject' => 'Subject',
    'contact.form.success_message' => 'Thank you for your message! We will get back to you as soon as possible.',
    'contact.form.type' => 'Are you a private individual or a company?',
    'contact.form.type_company' => 'Company',
    'contact.form.type_private' => 'Private',
    'contact.hours' => 'Opening Hours',
    'contact.hours_weekdays' => 'Monday to Friday 08:00 - 16:30',
    'contact.hours_weekend' => 'Helger och röda dagar 12:00 - 15:00',
    'contact.send_message' => 'Send message',
    'contact.subtitle' => 'We support your growth with recruitment and staffing – even at short notice',
    'contact.title' => 'Contact us',

    // Cookie
    'cookie.accept' => 'Accept',
    'cookie.consent_given' => 'Cookie consent saved.',
    'cookie.customize' => 'Customize',
    'cookie.privacy_policy' => 'Privacy Policy',
    'cookie.reject' => 'Reject',
    'cookie.settings.analytics.description' => 'Help us understand how visitors interact with our website.',
    'cookie.settings.analytics.title' => 'Analytics Cookies',
    'cookie.settings.cancel' => 'Cancel',
    'cookie.settings.description' => 'Choose which cookies you want to accept.',
    'cookie.settings.essential.description' => 'Required for the website to function properly.',
    'cookie.settings.essential.title' => 'Essential Cookies',
    'cookie.settings.marketing.description' => 'Used to deliver personalized advertisements.',
    'cookie.settings.marketing.title' => 'Marketing Cookies',
    'cookie.settings.save' => 'Save Preferences',
    'cookie.settings.title' => 'Cookie Settings',
    'cookie.text' => 'We use cookies to enhance your experience. By continuing to visit this site you agree to our use of cookies.',

    // Dashboard
    'dashboard.edit_profile' => 'Edit profile',
    'dashboard.email' => 'Email',
    'dashboard.expected_start' => 'Expected start',
    'dashboard.my_applications' => 'My applications',
    'dashboard.my_info' => 'My information',
    'dashboard.name' => 'Name',
    'dashboard.new_application' => 'New application',
    'dashboard.no_applications' => 'You have no applications yet.',
    'dashboard.no_cv' => 'No CV uploaded',
    'dashboard.quick_links' => 'Quick Links',
    'dashboard.search_jobs' => 'Search jobs',
    'dashboard.sent' => 'Sent',
    'dashboard.spontaneous_application' => 'Spontaneous Application',
    'dashboard.spontaneous_apply' => 'Spontaneous Application',
    'dashboard.status.accepted' => 'Accepted',
    'dashboard.status.pending' => 'Pending',
    'dashboard.status.rejected' => 'Rejected',
    'dashboard.status.reviewed' => 'Reviewed',
    'dashboard.status.shortlisted' => 'Shortlisted',
    'dashboard.table.actions' => 'Actions',
    'dashboard.table.application_info' => 'Application Information',
    'dashboard.table.applied_date' => 'Applied Date',
    'dashboard.table.close' => 'Close',
    'dashboard.table.cover_letter' => 'Cover letter',
    'dashboard.table.cv' => 'CV / Resume',
    'dashboard.table.download_cv' => 'Download CV',
    'dashboard.table.job' => 'Job',
    'dashboard.table.job_application' => 'Job Application',
    'dashboard.table.no_cover_letter' => 'No cover letter provided',
    'dashboard.table.no_cv' => 'No CV uploaded',
    'dashboard.table.start_date' => 'Start Date',
    'dashboard.table.status' => 'Status',
    'dashboard.table.type' => 'Type',
    'dashboard.table.view_details' => 'View Details',
    'dashboard.title' => 'My Profile',
    'dashboard.view_cv' => 'View CV',
    'dashboard.view_jobs' => 'View available jobs',
    'dashboard.welcome' => 'Welcome',

    // Employers
    'employers.consulting.description' => 'We handle the transition from staffing to direct employment – ​​quickly, easily and according to agreement',
    'employers.consulting.title' => 'Taking over employment',
    'employers.cta.button' => 'Send a request',
    'employers.cta.description' => 'Describe your needs in the form and we will get back to you within 24 hours',
    'employers.cta.title' => 'Looking for staff?',
    'employers.recruitment.description' => 'We help you find the right candidates — efficiently and in a structured way. We manage recruitment from start to goal',
    'employers.recruitment.title' => 'Recruitment',
    'employers.services.subtitle' => 'V U S offers professional staffing and recruitment',
    'employers.services.title' => 'Our services',
    'employers.staffing.description' => 'Access to  personnel for both short and long assignments in various industries.',
    'employers.staffing.title' => 'Staffing',
    'employers.subtitle' => 'We help you find the right employees',
    'employers.title' => 'For employers',
    'employers.why_choose.experience' => 'We offer a free staffing and needs analysis
',
    'employers.why_choose.fast' => 'We handle the entire recruitment and staffing process – from requirement profile to employment.',
    'employers.why_choose.network' => 'Flexibility and commitment permeate our collaboration
',
    'employers.why_choose.personal' => 'Efficient and tailored candidate matching',
    'employers.why_choose.support' => 'Ongoing follow-up and support',
    'employers.why_choose.title' => 'Why choose V U S?',

    // Footer
    'footer.copyright' => '© :year VUS Bemanning – Org.nr 559540-8963',
    'footer.follow_us' => 'Follow us',
    'footer.quick_links' => 'Quick links',

    // Home
    'home.broad_competence.description' => 'We have experience in many different industries and can help you with the right expertise.',
    'home.broad_competence.title' => 'Broad Competence',
    'home.cta.contact' => 'Contact us',
    'home.cta.description' => 'Whether you are looking for a job or seeking qualified staff, we are here to help you.',
    'home.cta.jobs' => 'View available jobs',
    'home.cta.title' => 'Ready to take the next step?',
    'home.employers.button' => 'Learn more',
    'home.employers.description' => 'Looking for qualified staff? We help you find the right employees for your needs.',
    'home.employers.title' => 'For employers',
    'home.fast_matching.description' => 'We work efficiently to quickly find the right employees for your business.',
    'home.fast_matching.title' => 'Effective matching',
    'home.find_job.button' => 'View available jobs',
    'home.find_job.description' => 'Search through our available positions and find the perfect match for your skills and experience.',
    'home.find_job.title' => 'Find the job that suits you',
    'home.hero.button_available_jobs' => 'Available jobs',
    'home.hero.button_for_employers' => 'For employers',
    'home.hero.location' => 'Lulea/Norrbotten',
    'home.hero.subtitle' => 'Professional staffing',
    'home.hero.title' => 'Welcome to V U S',
    'home.hero.title_prefix' => 'Welcome to',
    'home.hero.title_vus' => 'V U S Bemanning',
    'home.partners.subtitle' => 'Some of our customers and partners',
    'home.partners.title' => 'Cooperation Partners',
    'home.partners.visit_website' => 'Visit website',
    'home.professional_service.description' => 'We offer high-quality staffing and recruitment services for companies',
    'home.professional_service.title' => 'Professional Service',
    'home.register.button' => 'Register',
    'home.register.description' => 'Create an account and upload your CV to apply for jobs and receive notifications about new positions.',
    'home.register.title' => 'Register your profile',
    'home.reviews.review1.company' => 'BD Fisk AB ',
    'home.reviews.review1.text' => 'BD Fisk AB ',
    'home.reviews.review2.company' => 'Lulea Hockey restaurangerna',
    'home.reviews.review2.text' => 'Lulea Hockey restaurangerna',
    'home.reviews.review3.company' => 'Allstar Lulea',
    'home.reviews.review3.text' => 'Allstar Lulea',
    'home.reviews.subtitle' => 'What our clients say about us',
    'home.reviews.title' => 'Customer Reviews',
    'home.why_choose.title' => 'Why choose V U S?',

    // Jobs
    'jobs.deadline' => 'Application Deadline',
    'jobs.detail.already_applied' => 'You have already applied to this job. You can edit your application from your profile.',
    'jobs.detail.apply_now' => 'Apply now',
    'jobs.detail.description' => 'Description',
    'jobs.detail.edit_application' => 'Edit application',
    'jobs.detail.info' => 'Job information',
    'jobs.detail.location' => 'Location',
    'jobs.detail.login_required' => 'You need be logged in to apply',
    'jobs.detail.published' => 'Published',
    'jobs.detail.register' => 'Register',
    'jobs.detail.requirements' => 'Meritorious:',
    'jobs.detail.responsibilities' => 'Requirements',
    'jobs.detail.salary' => 'Salary',
    'jobs.detail.share' => 'Part-time',
    'jobs.detail.type' => 'Type',
    'jobs.filter.all' => 'All',
    'jobs.filter.button' => 'Filter',
    'jobs.filter.clear' => 'Clear',
    'jobs.filter.contract' => 'Time-limited',
    'jobs.filter.employment_type' => 'Employment type',
    'jobs.filter.full_time' => 'Full Time',
    'jobs.filter.location' => 'City',
    'jobs.filter.location_placeholder' => 'City',
    'jobs.filter.part_time' => 'Part-time',
    'jobs.filter.search' => 'Search',
    'jobs.filter.search_placeholder' => 'Job title...',
    'jobs.filter.temporary' => 'Hourly employment',
    'jobs.filter.title' => 'Filter jobs',
    'jobs.no_jobs.description' => 'There are currently no available services that match your search criteria at the moment.',
    'jobs.no_jobs.spontaneous' => 'Spontaneous Application',
    'jobs.no_jobs.title' => 'No jobs found',
    'jobs.read_more' => 'Read more',
    'jobs.subtitle' => 'Find your next career opportunity with us',
    'jobs.title' => 'Available jobs',

    // Nav
    'nav.about' => 'About us',
    'nav.blog' => 'Insights & news',
    'nav.candidate_information' => 'Employee info',
    'nav.company_values' => 'Our core values',
    'nav.contact' => 'Contact us',
    'nav.our_employees' => 'Management',
    'nav.for_employers' => 'For employers',
    'nav.for_job_seekers' => 'For job Seekers',
    'nav.free_services' => 'Our services',
    'nav.home' => 'Home',
    'nav.jobs' => 'Available jobs',
    'nav.login' => 'Log In',
    'nav.logout' => 'Log out',
    'nav.profile' => 'My profile',
    'nav.register' => 'Register',

    // Newsletter
    'newsletter.already_subscribed' => 'This email is already subscribed.',
    'newsletter.email_placeholder' => 'Enter your email',
    'newsletter.error' => 'An error occurred. Please try again.',
    'newsletter.subscribe_button' => 'Subscribe',
    'newsletter.subscribed' => 'Successfully subscribed to newsletter!',
    'newsletter.subtitle' => 'Get notified about new job openings',
    'newsletter.title' => 'Subscribe to newsletter',

    // Privacy
    'privacy.contact.text' => 'If you have any questions about this privacy policy, please contact us at abdulrazek.mahmoud@vus-bemanning.se',
    'privacy.contact.title' => 'Contact us',
    'privacy.cookies.text' => 'We use cookies to enhance your browsing experience. You can manage your cookie preferences through your browser settings.',
    'privacy.cookies.title' => 'Cookies',
    'privacy.data_collection.text' => 'We collect personal information that you provide to us when applying for jobs, including your name, email address, phone number, CV, and other relevant information.',
    'privacy.data_collection.title' => 'Data Collection',
    'privacy.data_usage.text' => 'We use your personal information to process job applications, match you with suitable positions, and communicate with you about opportunities.',
    'privacy.data_usage.title' => 'How we use your data',
    'privacy.introduction.text' => 'At V U S, we are committed to protecting your privacy. This Privacy Policy explains how we collect, use, and safeguard your personal information.',
    'privacy.introduction.title' => 'Introduction',
    'privacy.rights.text' => 'You have the right to access, correct, or delete your personal information at any time. You can also withdraw your consent for data processing.',
    'privacy.rights.title' => 'Your rights',
    'privacy.title' => 'Privacy policy',

    // Profile
    'profile.applications.actions' => 'Actions',
    'profile.applications.application_date' => 'Application Date',
    'profile.applications.applied_date' => 'Applied date',
    'profile.applications.back_to_list' => 'Back to applications',
    'profile.applications.basic_info' => 'Basic information',
    'profile.applications.cover_letter' => 'Cover letter',
    'profile.applications.current_cv' => 'Current CV',
    'profile.applications.cv_cover_letter' => 'CV & cover Letter',
    'profile.applications.cv_document' => 'CV document',
    'profile.applications.dates_timeline' => 'Dates & timeline',
    'profile.applications.download_cv' => 'Download CV',
    'profile.applications.edit' => 'Edit',
    'profile.applications.edit_application' => 'Edit application',
    'profile.applications.expected_start_date' => 'Expected start date',
    'profile.applications.job' => 'Job',
    'profile.applications.job_application' => 'Job application',
    'profile.applications.last_updated' => 'Last updated',
    'profile.applications.no_applications' => 'You have no applications yet.',
    'profile.applications.no_cover_letter' => 'No cover letter provided',
    'profile.applications.no_cv' => 'No CV uploaded',
    'profile.applications.optional' => 'Optional',
    'profile.applications.search_jobs' => 'Search Jobs',
    'profile.applications.spontaneous' => 'Spontaneous Application',
    'profile.applications.start_date' => 'Start Date',
    'profile.applications.start_date_option' => 'Start date option',
    'profile.applications.status' => 'Status',
    'profile.applications.status.accepted' => 'Accepted',
    'profile.applications.status.pending' => 'Pending',
    'profile.applications.status.rejected' => 'Rejected',
    'profile.applications.status.reviewed' => 'Reviewed',
    'profile.applications.status.shortlisted' => 'Shortlisted',
    'profile.applications.type' => 'Type',
    'profile.applications.update_application' => 'Update Application',
    'profile.applications.updated' => 'Application updated successfully.',
    'profile.applications.upload_new_cv' => 'Upload new CV',
    'profile.applications.view_cv' => 'View CV',
    'profile.applications.view_details' => 'View details',
    'profile.applications.current_documents' => 'Current Documents',
    'profile.applications.additional_document' => 'Additional Document',
    'profile.applications.upload_documents' => 'Upload Documents',
    'profile.applications.upload_documents_help' => 'You can upload up to 3 files total (including CV). Accepted formats: PDF, DOC, DOCX, Images. Maximum size: 3MB per file.',
    'profile.applications.view' => 'View',
    'profile.applications.download' => 'Download',
    'profile.applications.delete' => 'Delete',
    'profile.applications.confirm_delete_file' => 'Are you sure you want to delete this file?',
    'profile.current_cv' => 'Current CV:',
    'profile.cv' => 'CV / Resume',
    'profile.cv_description' => 'Upload or update your CV. This CV will be used for all your job applications.',
    'profile.cv_formats' => 'Accepted formats: PDF, DOC, DOCX. Maximum size: 5MB',
    'profile.cv_updated' => 'CV Updated.',
    'profile.delete_account' => 'Delete account',
    'profile.download_cv' => 'Download CV',
    'profile.password.confirm' => 'Confirm new password',
    'profile.password.current' => 'Current password',
    'profile.password.description' => 'Ensure your account is using a long, random password to stay secure.',
    'profile.password.new' => 'New password',
    'profile.tab.applications' => 'Applications',
    'profile.tab.applications_desc' => 'View your job applications',
    'profile.tab.profile' => 'Profile',
    'profile.tab.profile_desc' => 'Manage your profile information',
    'profile.title' => 'My profile',
    'profile.update_password' => 'Update password',
    'profile.update_profile' => 'Update profile Information',
    'profile.upload_new_cv' => 'Upload new CV',
    'profile.view_current_cv' => 'View current CV',
    'profile.welcome' => 'Welcome',

    // Spontaneous
    'spontaneous.consent' => 'I hereby consent that the information I provide in this application to be used to match me with potential jobs',
    'spontaneous.cover_letter' => 'Cover letter',
    'spontaneous.cover_letter_placeholder' => 'Tell us a little about yourself, your skills, and what type of work you are looking for.',
    'spontaneous.description' => 'Can\'t find a job that suits you right now? Submit a spontaneous application and we will contact you when a suitable position becomes available.',
    'spontaneous.file' => 'File',
    'spontaneous.login_required' => 'You must be logged in to submit a spontaneous application.',
    'spontaneous.register' => 'Register',
    'spontaneous.register_description' => 'Please create an account to submit your spontaneous application. All fields are required to complete your profile.',
    'spontaneous.send_title' => 'Send your spontaneous application',
    'spontaneous.submit' => 'Submit spontaneous application',
    'spontaneous.subtitle' => 'Submit your application even if there is no available position that suits you right now',
    'spontaneous.title' => 'Spontaneous Application',
    'spontaneous.upload_cv' => 'Upload CV (PDF, DOC, DOCX)',
    'spontaneous.upload_files' => 'Upload files',
    'spontaneous.upload_files_description' => 'You can upload up to 3 files (CV, certificates, etc.). The first file is required.',

    // Validation
    'validation.already_applied' => 'You have already applied to this job. You can edit your application from your profile page.',
    'validation.application_sent' => 'Your application has been sent!',
    'validation.cv_required' => 'CV file is required',
    'validation.cv_upload_error' => 'CV upload failed: :message',
    'validation.cv_upload_failed' => 'Failed to upload CV. Please try again.',
    'validation.date_of_birth_invalid' => 'Invalid date of birth',
    'validation.max_files_exceeded' => 'Maximum :max files allowed. Please remove some files or delete existing ones first.',
    'validation.spontaneous_sent' => 'Your spontaneous application has been sent!',

    // Blog
    'blog.title' => 'Blog',
    'blog.subtitle' => 'Latest news and updates from VUS',
    'blog.read_more' => 'Read more',
    'blog.back_to_blog' => 'Back to blog',
    'blog.no_posts' => 'No blog posts available at the moment.',
    'blog.recent_posts' => 'Recent posts',

    // Search
    'search.title' => 'Search',
    'search.placeholder' => 'Search the website...',
    'search.button' => 'Search',
    'search.results_for' => 'Search results for',
    'search.found_results' => 'Found :count result(s)',
    'search.no_results' => 'No results found',
    'search.try_different' => 'Try different keywords or check spelling',
    'search.enter_query' => 'Enter your search query above',
    'search.view_page' => 'View page',
    'search.type.page' => 'Page',
    'search.type.blog' => 'Blog Post',
    'search.type.job' => 'Job',
];
