            ...
            <div class="col-lg-8 col-12 mx-auto">
                <div class="custom-block bg-white shadow-lg p-3">
                    ...
                    <form method="POST" action="{{ route('profile.applications.update', $application->id) }}" enctype="multipart/form-data">
                        ...
                        <div class="mb-2">
                            <div class="form-check mb-2">
                                <input class="form-check-input checkbox-like-radio @error('start_date_option') is-invalid @enderror" type="radio" name="start_date_option" id="availability_immediately" value="immediately" {{ old('start_date_option', $application->start_date_option) == 'immediately' ? 'checked' : '' }} required>
                                <label class="form-check-label small" for="availability_immediately">
                                    {{ __('messages.apply.start_date_option.immediately') }}
                                </label>
                            </div>
                            <div class="form-check mb-2">
                                <input class="form-check-input checkbox-like-radio @error('start_date_option') is-invalid @enderror" type="radio" name="start_date_option" id="availability_one_month" value="one_month" {{ old('start_date_option', $application->start_date_option) == 'one_month' ? 'checked' : '' }} required>
                                <label class="form-check-label small" for="availability_one_month">
                                    {{ __('messages.apply.start_date_option.one_month') }}
                                </label>
                            </div>
                            <div class="form-check mb-2">
                                <input class="form-check-input checkbox-like-radio @error('start_date_option') is-invalid @enderror" type="radio" name="start_date_option" id="availability_two_three_months" value="two_three_months" {{ old('start_date_option', $application->start_date_option) == 'two_three_months' ? 'checked' : '' }} required>
